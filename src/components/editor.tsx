...
export function Editor({ value, onChange, placeholder = 'Bắt đầu viết...' }: EditorProps) {
  ...
  const handleLineSpacingChange = (spacing: string) => {
    ...
    if (editor) {
      editor.view.dom.style.lineHeight = spacing;
    }
  };
  ...
  return (
    <div className="rounded-lg border bg-card flex flex-col shadow-sm">
      {/* Main Toolbar */}
      <div className="p-2 flex items-center gap-0.5 flex-wrap border-b bg-muted/30 sticky top-0 z-10">
        {/* Headings Dropdown */}
        <DropdownMenu>
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="ghost"
              size="icon"
              className={`h-8 w-8 ${editor.isActive('link') ? 'bg-muted text-primary' : ''}`}
            >
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <div className="px-2 py-1 text-xs text-muted-foreground">Line Spacing</div>
            {lineSpacingOptions.map((option) => (
              <DropdownMenuItem
                key={option.value}
                onClick={() => handleLineSpacingChange(option.value)}
                className={lineSpacing === option.value ? 'bg-muted' : ''}
              >
                {option.label}
              </DropdownMenuItem>
            ))}
